...
# from Misc.ControlTable_H42P import ControlTable_H42P as CT_H42P
# from Misc.ControlTable_H42P import ControlTable_H42P as CT_H42P
# from Misc.ControlTable_L42 import ControlTable_L42 as CT_L42
# from Misc.ControlTable_L42 import ControlTable_L42 as CT_L42
...
from Misc.Helpers import int32, int16
...
class DCMotor:
    def __init__(self, DXL_ID, MODEL, BAUD=1e6, PORT=None, packet = None, port = None):
        self.DXL_ID = DXL_ID
        self.MODEL = MODEL
        assert MODEL in ["H42P", "L42", "M42"], "Model not recognized. Supported models: H42P, L42, M42"
        self.PORT = PORT if PORT is not None else get_tty_port()
        self.BAUD = BAUD;
        self.PROTOCOL_VER = 2;
        self.CT = CT_H42P if MODEL == "H42P" else CT_L42 if MODEL == "L42" else CT_M42
        # CT = self.CT
        if port is None:
    ...
    def getInfo(self, verbose = False):
        ...
        opmode, c,e = self.packet.read1ByteTxRx(self.port, self.DXL_ID, self.CT.OPERATING_MODE.value)
        match opmode:
    ...
    def setOpMode(self, opmode: OpModes, verbose=False):
        ...
        c, e = self.packet.write1ByteTxRx(self.port, self.DXL_ID, self.CT.OPERATING_MODE.value, opmode.value)
    ...
    def isMoving(self, verbose=True):
        status, c, e = self.packet.read1ByteTxRx(self.port, self.DXL_ID, self.CT.MOVING.value)
        if verbose:
    ...
    def setCurrentPosition(self, position, verbose = False):
        c, e = self.packet.write4ByteTxRx(self.port, self.DXL_ID, self.CT.CURRENT_POSITION.value, int(position))
        if verbose: print("Set Current Position:", position, "Response:", c, e)
    def getCurrentPosition(self, verbose = False):
        pos, c, e = self.packet.read4ByteTxRx(self.port, self.DXL_ID, self.CT.CURRENT_POSITION.value)
        # if e == 0:
    ...
    def getGoalPosition(self, verbose = True):
        goal, c, e = self.packet.read4ByteTxRx(self.port, self.DXL_ID, self.CT.GOAL_POSITION.value)
        if verbose: print("Goal Position:", int32(goal))
        return int32(goal)
    
    def getMaxPositionLimit(self, verbose = False):
        max_pos, c,e, = self.packet.read4ByteTxRx(self.port, self.DXL_ID, self.CT.MAX_POSITION_LIMIT.value)
        if verbose: print("Max Position Limit:", int32(max_pos))
        return int32(max_pos)

    def getMinPositionLimit(self, verbose = False):
        min_pos, c,e, = self.packet.read4ByteTxRx(self.port, self.DXL_ID, self.CT.MIN_POSITION_LIMIT.value)
    ...
    def setGoalPosition(self, goal, verbose = True):
        ...
        goal = int(goal)
        c, e = self.packet.write4ByteTxRx(self.port, self.DXL_ID, self.CT.GOAL_POSITION.value, goal)
        if verbose: print("write goal:", c,e, "goal:", goal)

    def getHomingOffset(self, verbose = True):
        offset, c, e = self.packet.read4ByteTxRx(self.port, self.DXL_ID, self.CT.HOMING_OFFSET.value)
        if verbose: print("Homing Offset:", int32(offset))
        return int32(offset)
    def setHomingOffset(self, offset, verbose = True):
        c, e = self.packet.write4ByteTxRx(self.port, self.DXL_ID, self.CT.HOMING_OFFSET.value, offset)
    ...
    def getTorque(self, verbose = True):
        tm, c, e = self.packet.read1ByteTxRx(self.port, self.DXL_ID, self.CT.TORQUE_ENABLE.value)
        state = "ON" if tm == 1 else "OFF"
        if verbose: print(f"Torque: {state}")
        return tm
    def enableTorque(self, verbose = True):
        c, e = self.packet.write1ByteTxRx(self.port, self.DXL_ID, self.CT.TORQUE_ENABLE.value, 1)
        if e == 0 and verbose: print("Torque enabled")
    def disableTorque(self, verbose = True):
        c, e = self.packet.write1ByteTxRx(self.port, self.DXL_ID, self.CT.TORQUE_ENABLE.value, 0)
        if e == 0 and verbose: print("Torque disabled")

    # DRIVE MODE (Address: CT.DRIVE_MODE)
    def getDriveMode(self, verbose=True):
        dm, c, e = self.packet.read1ByteTxRx(self.port, self.DXL_ID, self.CT.DRIVE_MODE.value)
        if verbose:
    ...
    def setDriveMode(self, value, verbose=True):
        ...
        if torque == 1:
            self.disableTorque(verbose=False)
        c, e = self.packet.write1ByteTxRx(self.port, self.DXL_ID, self.CT.DRIVE_MODE.value, value & 0xFF)
        if torque == 1:
            self.enableTorque(verbose=False)
        if verbose:
    ...
    def getCurrentVelocity(self, verbose = True):
        velo, c, e = self.packet.read4ByteTxRx(self.port, self.DXL_ID, self.CT.CURRENT_VELOCITY.value)
    ...
    def setGoalVelocity(self, velocity, verbose = False, force = False):
        ...
        c, e = self.packet.write4ByteTxRx(self.port, self.DXL_ID, self.CT.GOAL_VELOCITY.value, velocity)
        if verbose: print("Set Goal Velocity:", velocity, "Response:", c, e)

    # PWM:
    def getPWMLimit(self, verbose = False):
        limit, c, e = self.packet.read2ByteTxRx(self.port, self.DXL_ID, self.CT.PWM_LIMIT.value)
        if verbose: print("PWM Limit:", int32(limit))
        return int32(limit)
    
    def getGoalPWM(self, verbose = True):
        pwm, c, e = self.packet.read2ByteTxRx(self.port, self.DXL_ID, self.CT.GOAL_PWM.value)
    ...
    def setGoalPWM(self, pwm, verbose = True, force = False):
        ...
        c, e = self.packet.write2ByteTxRx(self.port, self.DXL_ID, self.CT.GOAL_PWM.value, pwm)
        if verbose: print("Set Goal PWM:", pwm, "Response:", c, e)
    
    def getGoalCurrent(self, verbose = False):
        current, c, e = self.packet.read2ByteTxRx(self.port, self.DXL_ID, self.CT.GOAL_CURRENT.value)
    ...
    def setGoalCurrent(self, current, verbose = False):
        ...
        c, e = self.packet.write2ByteTxRx(self.port, self.DXL_ID, self.CT.GOAL_CURRENT.value, int(current))
        if verbose: print("Set Goal Current:", current, "Response:", c, e)

    def getPresentCurrent(self, verbose = False):
        """Reads the present current from the motor."""
        current, c, e = self.packet.read2ByteTxRx(self.port, self.DXL_ID, self.CT.PRESENT_CURRENT.value)
        if verbose: print("Present Current:", int16(current))
        return int16(current)

    # Profile Velocty (Address: 560)
    def getProfileVelocity(self, verbose = False):
        velo, c, e = self.packet.read4ByteTxRx(self.port, self.DXL_ID, self.CT.PROFILE_VELOCITY.value)
        if verbose: print("Profile Velocity:", int32(velo))
        return int32(velo)
    def setProfileVelocity(self, velocity, verbose = True):
        c, e = self.packet.write4ByteTxRx(self.port, self.DXL_ID, self.CT.PROFILE_VELOCITY.value, int(velocity))
    ...
    def getProfileAcceleration(self, verbose = False):
        accel, c, e = self.packet.read4ByteTxRx(self.port, self.DXL_ID, self.CT.PROFILE_ACCELERATION.value)
        if verbose: print("Profile Acceleration:", int32(accel))
        return int32(accel)
    def setProfileAcceleration(self, acceleration, verbose = True):
        c, e = self.packet.write4ByteTxRx(self.port, self.DXL_ID, self.CT.PROFILE_ACCELERATION.value, int(acceleration))
        if verbose: print("Set Profile Acceleration:", acceleration, "Response:", c, e)

    # LED: 
    def getLEDColor(self, verbose = True):
        red, c, e = self.packet.read1ByteTxRx(self.port, self.DXL_ID, self.CT.LED_RED.value)
        green, c, e = self.packet.read1ByteTxRx(self.port, self.DXL_ID, self.CT.LED_GREEN.value)
        blue, c, e = self.packet.read1ByteTxRx(self.port, self.DXL_ID, self.CT.LED_BLUE.value)
    ...
    def setLEDColor(self, color: Colors, verbose = True):
        ...
        g = color[1]
        b = color[2]
        c,e = self.packet.write1ByteTxRx(self.port, self.DXL_ID, self.CT.LED_RED.value, r)
        c,e = self.packet.write1ByteTxRx(self.port, self.DXL_ID, self.CT.LED_GREEN.value, g)
        c,e = self.packet.write1ByteTxRx(self.port, self.DXL_ID, self.CT.LED_BLUE.value, b)
