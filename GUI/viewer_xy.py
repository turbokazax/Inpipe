...
# # from Logic.Telemetry import SimpleTelemetry
...
# # Sender = Sender(HOST, PORT, 1/100)
# # sniffer = Sniffer(HOST, PORT)
# # -----------------------------
...
# def drain_udp():
#     global cur_x, cur_y, cur_state, r_now, last_pkt_time, pkt_count, sniffer

#     parts = sniffer.drain_udp()   # already drains queue and returns newest
#     if parts is None:
#         return  # <-- KEY: do not loop forever
...
# #         parts = sniffer.drain_udp()
# #         if parts is None:
# #             print("Got none. Stuck.")
# #             continue
...
#         state_text.set_text(f"waiting for UDP packets on port {PORT}...")
#     else:
...
#     sniffer = Sniffer(args.host, args.port)
#     sniffer.visualize_xy(axis_mode="dynamic", display_radius_init=deg(270))
...
import matplotlib.pyplot as plt
from matplotlib.animation import FuncAnimation
from Logic.Telemetry import Sniffer
import argparse

# AXIS_MODE = "dynamic"
DISPLAY_RADIUS_INIT = deg(270)
DYNAMIC_MARGIN = 1.10
TRAIL_POINTS = 100_000

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--axis_mode", default="dynamic")
    ap.add_argument("--r0", type=int, default = DISPLAY_RADIUS_INIT)
    ap.add_argument("--host", default="127.0.0.1")
    ap.add_argument("--port", type=int, required=True)
    args = ap.parse_args()
    ...
    cur_state = "?"
    r_now = None
    r_fixed = DISPLAY_RADIUS_INIT if args.r0 is None else args.r0
    axis_set = False
    last_pkt_time = None
    pkt_count = 0
    ...
    (circle_line,) = ax.plot([], [])
    (trail_line,) = ax.plot([], [])
    (marker_line,) = ax.plot([], [], marker="o", linestyle="")
    state_text = ax.text(0.02, 0.98, "", transform=ax.transAxes, va="top")
    ...
    def consider_axes_update(rr: float):
        nonlocal r_fixed, axis_set, cur_x, cur_y
        if not axis_set and r_fixed is not None:
            set_axes_for_radius(r_fixed)
            axis_set = True
        ...
        point_r = max(abs(cur_x), abs(cur_y)) if (cur_x is not None and cur_y is not None) else 0.0
        target_r = max(rr if rr is not None else 0.0, point_r, 1.0)
        set_axes_for_radius(target_r)

    def drain_udp():
        nonlocal cur_x, cur_y, cur_state, r_now, last_pkt_time, pkt_count
    ...
    def update(_frame):
        ...
        if last_pkt_time is None:
            state_text.set_text(f"waiting for UDP packets on port {args.port}...")
        else:
            age = time.time() - last_pkt_time
            state_text.set_text(
                f"axis_mode: {AXIS_MODE}\n"
    ...
    # KEEP REFERENCE or it gets GC'd
